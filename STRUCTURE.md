...
- `ensure-agent-home-ownership`: runs a one-shot root helper container from the configured Hermes image and recursively assigns an agent home volume to that image's dynamic `hermes` UID/GID when needed. Its optional `--run-hermes-update` mode is reserved for package update hooks and runs `hermes update` as the image `hermes` user after ownership repair.
